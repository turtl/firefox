...
var init	=	function(rsa, options)
{
	...
	panel.port.on('notify-rsa', function(on) {
		rsa.notify_on_gen(!!on);
	});

	panel.on('show', function() { panel.port.emit('open'); });

	var menu_rsa_gen	=	function() { panel.port.emit('rsa-gen'); };
	var menu_rsa_new	=	function() { panel.port.emit('rsa-key'); };
	on(rsa.port, 'gen-key', menu_rsa_gen);
	on(rsa.port, 'new-key', menu_rsa_new);
	...
	{
		panel.destroy();
		off(rsa.port, 'gen-key', menu_rsa_gen);
		off(rsa.port, 'new-key', menu_rsa_new);
	};
