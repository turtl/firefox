(function() {
	...
	// -------------------------------------------------------------------------
	/**
	 * Opens the tagit app tab (assuming it's not already open) and tracks/loads
	 * it.
	...
	/**
	 * Make sure that there are no open tagit tabs. This essential, because an
	 * open tagit tab (when logged out) will really screw many things up for the
	 * user.
	 *
	...
	var monitor_app_tabs	=	function()
	{
		...
		for(var i = 0, n = tablist.length; i < n; i++)
		{
			...
			// if we find an open tagit tab, close it
	...
	/**
	 * Shows (or hides) a loading indicator in the tagit button logo.
	 */
	...
	/**
	 * Sets up a handler which listens to the main menu events and dispatches
	 * accordingly.
	 */
	...
	exports.main	=	function(options, callbacks)
	{
		...
		poller.set_enable_cb(function() {
			return enabled;
		});
		poller.poll();

		// hook into the main button/menu
		menu_event_handler();

		// make sure no tagit tabs are open unless logged in
		monitor_app_tabs();
