(function() {
	...
	var open_app	=	function(options)
	{
		...
		var tab_ready	=	function(tab)
		{
			...
			// sync to app
			var sync_to_app	=	function(data, forced) {
				worker.port.emit('profile-sync', data, forced);
			};
	...
	var do_login	=	function(user_id, user_auth, user_key, options)
	{
		...
		background.panel_port().on('invite-remove', function() {
			menu.port.emit('num-invites', invites.num_pending());
		});
	...
	var setup_menu	=	function(type)
	{
		...
		default:
			...
			menu.button().buttons(function(btn) {
				btn.setAttribute('image', data.url('app/favicon.gray.png'));
	...
	exports.main	=	function(options, callbacks)
	{
		...
		invites.init(data_handler, background);
